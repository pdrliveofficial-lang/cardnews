...
6. **스레드 글 포맷 v2 (2026-08-07 사장님 지시)**: "[사연 접수]" 제보 프레임 제거 — 1인칭으로 바로 시작. 설문은 두 계정 모두 예/아니요 통일(마무리 질문을 예/아니요로 답할 수 있게 쓸 것). **가장 궁금한 대목은 `||...||`로 감싸면 발행 시 탭해서 보는 블라인드(스포일러)로 변환**됨 (post_threads.py parse_spoilers → text_entities API, UTF-16 오프셋, 글당 최대 10개, 거부 시 자동 폴백). 신규 풀 작성 시 이 포맷 준수.
