...
### ⚠️ 크롬 뷰포트 버그 (2026-09-11 수정)

2026-09 크롬 업데이트 이후 `--window-size=1080,1350` 을 줘도 실제 뷰포트가 87px쯤 짧아져
카드 아래에 흰 띠가 생긴다. **판사·연구소 기존 템플릿도 지금 다시 렌더링하면 똑같이 깨진다.**
`render.py` 가 `SHOT_H=1500` 으로 넉넉히 띄운 뒤 Pillow로 1080×1350 으로 잘라내도록 고쳤으니,
옛 카드를 다시 뽑을 일이 있으면 그대로 쓰면 된다. Pillow가 없으면 경고만 찍고 넘어간다.

### 결혼준비 카드 (template-wedding.html) — 2026-09-11 추가

`@serendi_pity_` (세렌디피티 | 결혼준비) 인스타용. slug가 `wed` 로 시작하면 이 템플릿이 선택된다.
사연·판결이 아니라 **"정리된 자료"** 톤이라 카드 타입이 완전히 다르다.

- 카드 타입: `cover`(표지) / `points`(번호목록, items[{n,t,d}]) / `price`(가격표, rows[{item,range,note}])
  / `warn`(테두리 경고박스, title+body) / `tip`(한 줄 핵심, big+sub) / `cta`(마지막 유도 카드)
- 색: 아이보리 `#FBF9F5` + 먹색 `#1C1A17` + 황동 `#96763F`. 기존 인스타의 흑백 빈티지 그리드와
  나란히 놓여도 안 튀도록 단색 악센트 하나만 쓴다.
- **표지는 padding이 다르다** (`.card.cover .inner`): 인스타 그리드가 4:5를 1:1 중앙 크롭으로
  보여줘서 위아래 135px씩 잘린다. 로고와 도장이 그리드에서도 보이도록 안쪽으로 들여썼다.
- 모든 세트의 마지막 카드는 `댓글에 "체크리스트" 남기면 보내드릴게요`로 끝난다 —
  나중에 매니챗 댓글→DM 자동화를 그대로 붙이기 위한 구조.
- 발행: 계정이 아직 **개인 계정**이라 `upload_instagram.py` (API)를 못 쓴다. 수동 업로드.
  프로페셔널(비즈니스/크리에이터) 전환 + 페이스북 페이지 연결을 하면 API 업로드와 매니챗이
  한꺼번에 풀린다. 전환 후 `IG_TOKEN`/`IG_USER_ID` 를 이 계정 것으로 새로 발급해야 한다.
...
- **⚠️ 전환 실패 교훈 (8/8)**: 연구소는 누적 조회 50만인데 스레드 팔로워 45명·인스타 0명(전환율 0.009%). 카풀 글이 9.9시간에 조회 36,635·좋아요 1,005·댓글 1,031을 찍었지만 ①답글 못 담(페이지네이션 버그) ②CTA 없음 ③프로필에 볼 것 없음으로 전부 흘려보냈다. **터진 글에는 즉시 답글 총력**(답글 알림이 댓글 단 사람 전원에게 가므로 가장 강한 재노출 수단) + 답글 CTA로 팔로우 유도할 것.
  - ⚠️ **셀프 홍보 답글 고정(핀)은 금지** — 8/8에 내가 "터진 글에 홍보 댓글 고정"을 제안했다가 철회. ①8/2 사용자 지시("셀프 답글 하지 마라")와 정면 충돌 ②카풀 글은 본문에 "좋아요 많은순으로 고정댓글됩니다!"라고 공개 약속을 걸고 실제로 독자 댓글(♥4.6천)을 고정해둔 상태라, 우리 홍보로 바꾸면 2,500 댓글러와의 약속을 깨는 것. **핀 자리는 독자 댓글용이다.**
...
- **🛑 댓글판사(@comment.judgee)도 운영 중단 (2026-08-21 사장님 결정) — cardnews 저장소 전체 휴면**
  - **근거**: 8/12 조치(답글봇 감속·engagement bait 제거) 후에도 **9일간 회복 없음**. 8/15~8/20 스레드 글이 전부 조회 0~6뷰. 같은 기간 결혼봇은 조회 7.8만~13만/일로 정상이므로 플랫폼 문제가 아니라 이 계정에 걸린 제재로 확정.
  - **중단 범위**: `publish.yml`(캐러셀) 잡 전체 / `threads-extra.yml` Judge 스텝 / `reply.yml` 판사 답글 / `publish-reel.yml` 릴스 전체. 전부 `if: false` 가드이므로 지우면 즉시 복구.
  - **집중 대상 이전**: 세렌디피티(@serendi_pity_) + 양재꽃언니(@serendist.log) 두 계정. 작업은 `threads-bot` 저장소에서 진행한다.
  - **판사 자산 활용**: 히트작 상위권이 전부 결혼·경조사 소재(축의금·청첩장·하객·집들이)였다. `stories/case-*.json`과 `threads_pool_judge.json`의 소재를 **결혼봇 논쟁형 사연 슬롯(12:00)으로 이식**하는 게 가장 싼 충전 방법 — 8/22~28분에 이미 7편 적용했다.

- **🛑 잘잘못연구소(@jaljalmot.lap) 운영 중단 (2026-08-12 사장님 결정)**
  - **배경**: 8/9 인스타 발행 차단(`action is blocked`)이 4일째 미해결 + 스레드 도달 붕괴. 두 계정을 같은 앱·같은 패턴으로 돌리다 동반 제재된 구조라, **판사 하나로 집중**하고 리소스를 결혼봇·꽃 계정으로 돌리기로 결정.
...
- **🚨 2026-08-12 계정 도달 붕괴 사고 (원인·조치 기록 — 반드시 숙지)**
  - **증상**: 8/9~8/11 판사·연구소 스레드 글이 **조회 0~10뷰, 댓글 0개**로 붕괴 (직전 8/2~8/7은 평균 13,000~17,000뷰 / 최고 167,760뷰). 같은 시기 연구소 인스타는 발행 API가 `403 action is blocked`로 4일째 차단.
  - **대조군**: 같은 시기 결혼봇(@serendi_pity_, 다른 Meta 앱·다른 답글봇)은 조회 47,000~101,000/일로 **정상**. → 플랫폼 전체 문제가 아니라 **이 두 계정에 걸린 제재**로 판단.
  - **직전 변경(8/8 밤)**: 답글봇 정기 상한 25 → 80 → **150**, 답글 간격 2배 가속. 하루 8회 실행이므로 **최대 1,200답글/일/계정**. 자동 대량 답글은 Meta의 대표적 스팸 시그널.
  - **조치(8/12)**: 상한 150 → **15**, 스캔 20 → 8, 답글 간격 4s → **12s**. 팔로우·스하리 요구 문구(engagement bait) 68건 제거. 발행 실패 시 claim 롤백(`unclaim.py`) 추가.
  - **⚠️ 재발 방지 원칙**: ①답글봇 상한을 올릴 때는 **하루 총 발송량**(상한 × 실행횟수)으로 계산할 것 — 계정당 100건/일을 넘기지 말 것 ②"팔로우/좋아요/리포스트/스하리 주세요" 류 직접 요구는 engagement bait로 downrank 위험 ③지표가 며칠 이상 비정상이면 **먼저 자동화 볼륨부터 의심**할 것.
  - **회복 확인법**: `gh workflow run insights.yml` 후 로그의 조회 평균을 본다. 정상 복귀 기준 = 신규 글이 1,000뷰 이상. 회복까지 통상 3~14일.

- **📐 스레드 원고 포맷 확정 (2026-08-08 밤, 사용자 지시 — 판사·연구소 공통)**:
  ```
  <바로 본론. 3인칭 제보 서술. [사연 접수] 같은 헤더 붙이지 말 것>
  ||핵심 대사·반전은 블라인드 처리||
  <상대 입장도 일리 있게 한 줄 — 이 균형 문장을 절대 빼지 말 것>

  <공감 유도 마무리 질문 — "이건 너무한 거 맞죠?" 식으로 살짝 기울여서>

  스하리 남겨주면 다 갈게요   ← 스하리 유도로 마무리 (3종 로테이션)
  ```
  - **예/아니요 설문 전면 폐지** — 반응은 댓글로만 받는다. 중요한 건 설문보다 **블라인드 처리**라는 사용자 판단. `threads_poll` 필드와 pool 아이템의 `poll` 키는 모두 제거됨(아이템은 dict가 아닌 문자열). post_threads.py의 poll 지원 코드는 남아 있지만 호출되지 않는다.
  - **⚠️ 일괄 변환 시 함정**: "판결은 댓글로 —" 를 지울 때 `\s*` 로 앞뒤 공백까지 먹는 regex를 쓰면 **앞줄과 뒷줄이 붙어버려 균형 문장이 통째로 사라진다** (8/8 실사고 — 되돌리고 재작업). 반드시 **줄 단위**로 치환하고, 변환 후 "원본의 서술 문장이 전부 살아있는지" 검사할 것.
