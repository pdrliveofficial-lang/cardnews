...
6. **소개글 (2026-08-02)**: 두 계정 bio를 각 컨셉·발행시각에 맞춰 작성. 연구소는 수집/사건파일 어휘, 판사는 재판 어휘(판결/개정)로 차별화.
   ...
   - 판사 @comment.judgee — **인스타는 제보 프레임으로 갱신 완료 (2026-08-06)**: "제보로 들어온 사건들을 전달해드립니다 ⚖️ / 판결은 여러분의 댓글로 / 매일 낮 12시 개정 · 사연 제보는 DM (2026-08-06 "전달자" 포지션으로 재수정)". **스레드 bio는 구버전**("연애하다 생긴 애매한 싸움을 판결합니다...", 8/4 입력) — 현재 브라우저 스레드 세션이 연구소라 갱신 불가. 판사 스레드 로그인 시 위 문구로 교체할 것 (셀프 홍보 답글 삭제와 같은 대기 건).
